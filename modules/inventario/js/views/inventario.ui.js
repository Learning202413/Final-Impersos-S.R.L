 ...
 * Lógica global de UI para Inventario (Estandarizada con Admin).
 * Incluye Sidebar, Toasts y Modales de Confirmación.
 */
...
const UI = (function() {
    ...
    function hideModal(modalId) {
        ...
        // Limpieza de contenido para evitar estados sucios o duplicados
        if (container && (
            modalId === 'confirm-modal-container' || 
            modalId === 'item-modal-container' || 
            modalId === 'oc-modal-container' ||
            modalId === 'receive-modal-container'
        )) {
             container.innerHTML = ''; 
        }
    }

    // --- 3. Modal de Confirmación (Estilo Admin - Escudo Rojo) ---
    function showConfirmModal(title, messageOrHtml, confirmButtonText, callback) {
        ...
        // Detectar si es HTML o Texto plano para ajustar márgenes
        const isHtmlContent = /<[a-z][\s\S]*>/i.test(messageOrHtml);
        const messageClass = isHtmlContent ? 'mb-6' : 'mb-8 text-gray-600';

        const modalHtml = `
            <div class="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-8 text-center transform scale-100 animate-fade-in border border-gray-100 relative">
