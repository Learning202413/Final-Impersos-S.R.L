 ...
 * Servicio de Operador de Prensa con Trazabilidad Completa.
 */
...
export const OperadorService = {
    ...
    async startPreparation(ordenId) {
        ...
        // --- NUEVO LOG ---
        log('INICIO_PREPARACION', `Setup de máquina iniciado (Placas/Tintas/Papel) para OT ${ordenId}`);
        // -----------------
    ...
    async startPrinting(ordenId) {
        ...
        // --- NUEVO LOG ---
        log('INICIO_IMPRESION', `Producción (Tiraje) iniciada para OT ${ordenId}`);
        // -----------------
    ...
    async finishJob(ordenId, consumo, desperdicio) {
        ...
        // --- NUEVOS LOGS ---
        if (!errPrensa && !errPost) {
            log('ENVIO_A_POSTPRENSA', `Material impreso liberado y enviado al área de Acabados.`);
        }
